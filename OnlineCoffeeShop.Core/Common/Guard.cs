...
using OnlineCoffeeShop.Domain.Aggregates;
...
public static class Guard
{
    ...
    public static void AgainstZeroDecimal<TException>(decimal value, string name = "Value")
    where TException : BaseDomainException, new()
    {
        if (value > 0)
        {
        ...
        ThrowException<TException>($"{name} cannot be 0.");
    }

    public static void AgainstZeroNegativeInteger<TException>(int value, string name = "Value")
where TException : BaseDomainException, new()
    {
        if (value > 0)
        {
        ...
        ThrowException<TException>($"{name} cannot be 0 or negative.");
    }
